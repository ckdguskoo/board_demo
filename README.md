...
>게시판을 좀 꾸미기 위해 연한파란색과 연보라색을 적절하게 사용하여 배경색을 채워주고, 애니매이션 효과를 넣을것, 마우스 포인트 이동에따른 간단한 효과도 추가, 또 페이징 기능을 추가할것 한 페이지에 10개의 게시글만 보이도록
...
- cors문제
- 데이터베이스 연동 문제
- git actions가 제대로 안돌아가는 문제
- Ec2를 시작할때 마다 IP를 다시 입력해줘야하는 번거로움
- 새로 시작시 프론트엔드 이미지를 로컬에서 빌드해서 보내줘야하는 문제

---

## 문제 해결
- Cors문제 : WebConfig 클래스 추가해 해결
- 데이터베이스 문제 : 기존 데이터베이스 포트번호로 중복으로 인한 오류, 기존 데이터베이스 이미지삭제 및 포트번호 변경 
- git actions문제 : git actions 사용 보류, 쉘스크립트로 반 자동화 배포
- ..
- ..